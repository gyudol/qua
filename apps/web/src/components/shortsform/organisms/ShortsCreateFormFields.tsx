...
import VideoUploader from "../molecule/VideoUploader";

function ShortsCreateFormFields({
  payload,
  setPayload,
}: {
  payload: Omit<PostShortsReq, "media"> & Partial<Pick<PostShortsReq, "media">>;
  ...
  const validatePayload = (
    updatedPayload: Omit<PostShortsReq, "media"> &
      Partial<Pick<PostShortsReq, "media">>,
  ) => {
    const res = shortsFormSchema.safeParse(updatedPayload); // Zod 스키마 검증
    if (!res.success) {
  ...
  const handleVisibilityChange = (checked: boolean) => {
    ...
    const updatedPayload: Omit<PostShortsReq, "media"> &
      Partial<Pick<PostShortsReq, "media">> = {
      ...payload,
    ...
    <fieldset className="flex flex-col gap-4 h-full">
      ...
      <div className="flex flex-col space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {payload.hashtags.map((tag: Hashtag, index: number) => (
            <div
              key={`index-${tag.name}`}
      ...
      <VideoUploader setPayload={setPayload} />
    </fieldset>
  );
}

export default ShortsCreateFormFields;
