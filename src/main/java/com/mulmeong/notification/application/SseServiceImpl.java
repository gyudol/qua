...
public class SseServiceImpl implements SseService {
    ...
    public void send(NotificationHistory notificationHistory) {
        String receiverUuid = notificationHistory.getTargetUuid(); //알림 대상자
        String eventId = makeTimeIncludeId(receiverUuid); //이벤트 생성 시간
        MemberDto memberDto = feignService.getCompactProfileByUuid(notificationHistory.getSourceUuid());
        ...
        emitters.forEach(//알림 전송
                (key, emitter) -> {
                    ...
                    sendNotificationToClient(
                            emitter, eventId, key, NoticeResponseDto.toDto(notificationHistory, memberDto)
                    );
                }
        );
    }
